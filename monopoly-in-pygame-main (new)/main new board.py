...
class Property:
    ...
    def getPrice(self): # Returns the price of a property, based on its position on the board.
        if self.name == 'India':
            ...
            return 200
        # elif self.colour == 9: -> utility 숫자가 9임
        #     return 150
...
class AI: # There's an Eve object of the Player class and an AI object of the AI class. I like to think of 'Eve: Player' as Eve's body and 'AI: AI' as her brain.
    ...
    def wantsProp(self, prop): # Returns boolean of whether she wants to buy a given property or not.
        ...
        if prop.getPrice() <= Eve.money * 0.6:
            return True
        return False # 자꾸 이상한 말 뜨길래 혹시나 해서 넣어봄 -> 의미 없음.
...
# -------------------------------------------------------------------------------
# CHOOSE ENEMY AND TUTORIAL

chooseEnemy = True
chooseAlert = Alert('Choose Enemy', 'AI? or Player?')
example = ''
...
while chooseEnemy:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
                if chooseAlert.confirmOrDeny() == 'confirmed':
                    if chooseAlert.heading == 'Choose Enemy':
                        chooseEnemy = False
                        break
                if chooseAlert.confirmOrDeny() == 'denied':
                    if chooseAlert.heading == 'Choose Enemy':
                        chooseEnemy = False
                        break
    chooseAlert.write()
...
while not (winner==Eve or winner==user):
            ...
            if alert.confirmOrDeny() == 'confirmed':
                ...
                if type(alert) == EveAlert:
                    ...
                    if alert.heading == 'Fictional Trip':
                        ...
                        a = random.choice([num for num in range(36) if num not in exclude_nums])
                        squareNames = ['Go', 'France', 'Community Chest', 'England',
                                'North Station', 'Thailand', 'Chance', 'Turkey', 'Iran', 
                                'Jail', 'Congo', 'Germany','Vietnam', 'East Station', 'Egypt', 'Community Chest',
                                'Philippines', 'Ethiopia', 
                                'Free Parking', 'Japan', 'ChancEvesia', 'Community Chest',
                                'America', 'West Station', 'India', 'Super Tax', 'China']
                        
                        if a<27:
                            Eve.money+=200
                            
                        Eve.name = squareNames[a]
                        Eve.boardpos = a
    ...
    if user.isTurn and (beginning or not((type(alert) == EveAlert) or alert.heading == 'Memories from 2008')):
        ...
        # if user.inJail and not alert.heading == 'They see me rollin\'': # 원본
        #     user.boardpos = 9
        #     if user.jailTurns >= 3:
        #         if len(user.getOutOfJailFreeCards) > 0:
        #             alert = Alert('Get out of Jail Free?', 'Do you wish to use a #get out of jail free card?')
        ...
        if Eve.normalGameplay and alert.heading != 'Another one bytes the dust' and alert.heading != 'Stayin\' alive':
            ...
            if Eve.timeMoving == 0:
                ...
                # if alert.confirmed and Eve.money > 0 and type(alert) != Auction and not Eve.inJail:
                    ...
                    for i in range(15):
                        for street in streets:
                            if (prop.boardpos == user.boardpos) and street[0].streetOwned and street[0].owner == Eve and streets.index(street) <= 7:
